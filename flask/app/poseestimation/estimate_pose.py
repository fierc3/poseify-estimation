...
def estimate_pose_for_video(file_dir, user_id, guid, file_extension, new_file_extension=False, scale_fps=False):
    ...
    file_user_guid = file_user_guid.replace("//", "/")
    ...
    #wdir='/workspaces/poseify/src/Core/PoseEstimation/vp3d'
    wdir='/workspaces/poseify-estimation/flask/app/poseestimation/vp3d'
    ...
    command = [
    "python", f"{wdir}/inference/infer_video_d2.py",
    ...
    try:
        #command = f"bash -c 'cd vp3d && python run.py -d custom -k {guid} -arc 3,3,3,3,3 -c checkpoint --evaluate pretrained_h36m_detectron_coco.bin --render --viz-subject {guid}.{file_extension} --viz-action custom --viz-camera 0 --viz-video {input_video_location} --viz-export {estimation_result_location} --viz-output {estimation_result_location}.mp4 --viz-size 6'"
        command = f"bash -c 'cd vp3d && python run.py -d custom -k {guid} -arc 3,3,3,3,3 -c checkpoint --evaluate pretrained_h36m_detectron_coco.bin --render --viz-subject {guid}.{file_extension} --viz-action custom --viz-camera 0 --viz-video {input_video_location} --viz-export {estimation_result_location}'"
        print('------------------------------')
        ...
        print("Standard Output:", p2.stdout)
        # If you expect some output, print it here
